  ...
  <main class="page-shell grid min-h-screen place-items-center px-5 py-10">
    <div class="w-full max-w-6xl">
      <header class="mb-10 flex items-center justify-between">
        ...
        >
          <span :class="state.language === 'en' ? 'font-black text-stone-900' : 'font-normal text-stone-400'">EN</span>
          <span class="text-stone-300">/</span>
          <span :class="state.language === 'th' ? 'font-black text-stone-900' : 'font-normal text-stone-400'">TH</span>
        </button>
      </header>

      <section class="section-card p-8 text-center md:p-12">
        <p class="mx-auto mb-4 w-fit rounded-full bg-pale px-4 py-2 text-sm font-bold text-brand">
        ...
        <div class="mt-10 grid gap-5 md:grid-cols-3">
          ...
          <article v-for="card in cards" :key="card.title" class="rounded-3xl border border-stone-100 bg-white p-6 text-left shadow-soft">
            ...
            <p class="mt-2 min-h-12 text-sm leading-relaxed text-muted">{{ card.description }}</p>
            <RouterLink :to="card.to" class="primary-btn mt-6 block text-center">{{ card.button }}</RouterLink>
            <p class="mt-3 flex items-center gap-2 text-xs font-semibold text-muted">
              ...
              <Lock v-if="card.note.includes('Login') || card.note.includes('เข้าสู่ระบบ')" :size="14" />
              {{ card.note }}
            </p>
          </article>
        </div>
